#!/usr/bin/env python3
"""Generate Commons Tragedy experiment directories from anywhere."""
...
def generate_personas(
    ...
    if env_file is not None:
        ...
        for candidate in candidate_files:
            if candidate.is_file() and load_dotenv(candidate):
                loaded = True
                break
        if not loaded:
            load_dotenv()
...
"""
                ...
                env_block = f"""for ENV_FILE in \"{output_dir}/.env\" \"{output_dir.parent}/.env\" \"$ROOT_DIR/.env\"; do
  if [[ -f \"$ENV_FILE\" ]]; then
    set -a
    source \"$ENV_FILE\"
    set +a
...
\"$PY_BIN\" \"$ROOT_DIR/scripts/run_ct_experiment.py\" \\
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a CT experiment directory with config.yaml and steps.yaml"
    )
...
def main() -> None:
    ...
    personas = generate_personas(args.num_agents, env_file, repo_root)
    config = build_config(
    ...
    run_script_path.write_text(
        build_run_script(repo_root=repo_root, output_dir=output_dir, env_file=env_file),
        encoding="utf-8",
    )
